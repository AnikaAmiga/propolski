...
class Model_User extends Model
{
	...
	// 	var_dump($_SESSION);
	// 	if (empty($this->login) || empty($this->email) || empty($this->first_name)  || 
	// 	    empty($this->last_name) || empty($this->avatar) ) {
	// 		self.setAllFieldsFromDb();
		...
		public function editUser () {
				...
				$fields4validate=["login"=>$form_login, "email"=>$form_email];
					if (isset($form_first_name) && !empty($form_first_name)) {
						// array_push($fields4validate, "first_name"=>$form_first_name);
